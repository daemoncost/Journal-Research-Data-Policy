...
def _load_question_types() -> dict:
    ...
    :return: Dictionary where keys are question numbers (int), values are `True` if open, `False` otherwise.
    """
    with pkg_resources.open_text("daemon_analysis_tools.metadata", "question_type.yaml") as file:
...
def _get_explanation_column(data: pd.DataFrame, index: int) -> str:
    ...
    if next_column and "Please add the text from the RDP that supports your answer below:" in next_column:
        return next_column
    return None

def _initialize_question(grouped_questions: dict, publisher: str, journal: str, q_num: int, column: str):
    """Initializes a Question object in the grouped_questions dictionary if not already present."""
...
def _process_group(group: pd.DataFrame, grouped_questions: dict, publisher: str, journal: str, data: pd.DataFrame):
    ...
    for i, column in enumerate(data.columns):
        if _is_question_column(column):
            ...
            for idx, answer in enumerate(group[column]):
                explanation = group[explanation_col].iloc[idx] if explanation_col in group.columns else ""
                grouped_questions[publisher][journal][q_num]._add_answer(answer, explanation)
