...
def flatten_grouped_questions(
    ...
    :param grouped_questions: nested dict:
    publisher -> journal -> question_id -> Question
    ...
    for publisher_name, journals in grouped_questions.items():
        pub_meta = (
            publisher_metadata.get(publisher_name, {}) if publisher_metadata else {}
        )

        for journal_name, questions in journals.items():
            jour_meta = (
                journal_metadata.get(journal_name, {}) if journal_metadata else {}
            )
    ...
    cols = [
        "publisher",
        *sorted(pub_meta.keys()),
        "journal",
        *sorted(jour_meta.keys()),
        "question_id",
        "question_text",
        "is_open",
        "respondent_id",
