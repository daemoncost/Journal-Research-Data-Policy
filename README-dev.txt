If you are interested in contributing and do not know where to start take a look at the open issues.
https://github.com/daemoncost/Journal-Research-Data-Policy/issues
...
The raw data from the coding framework is collected in data/raw/rdp.csv, in case you want to add additional data this is the place.
When adding data ensure that emails are removed.
The data can be further processed as shown in exploration/example.ipynb. 
The example demonstrates how to use the tools provided in build/daemon_analysis_tools/ to correct inconsistencies
in the coded data generated by two different researchers. Based on the context the correct answer and the reason for the inconsistency can be noted down
resulting in the .yaml files in data/processed.

The YAML files can then be further analyzed. For any analysis scripts you write, create your own folder called NAME-analysis within data/analysis
or jupyter-notebooks in notebooks/exploration. The folder should include a changelog describing what kind of analysis you have already completed or are planning to do.
All analysis scripts should be developed under the assumption that the input data may continue to change in the future. 
notebooks/exploration/analysis_example.ipynb demonstrates how to load the yaml files to create some simple pie charts.
